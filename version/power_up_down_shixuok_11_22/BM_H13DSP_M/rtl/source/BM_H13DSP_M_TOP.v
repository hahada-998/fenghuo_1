...
//--------------------------------------------------------
//Device Number 
`define NUM_CPU 2'h02		//表示系统为双 CPU 配置（支持两个 CPU 同时工作）。
...
(
    .iClk           ( clk_50m            ),	// 将外部的 50MHz 时钟信号 clk_50m 连接到模块的 iClk 输入端口，作为基准时钟
    .iRst           ( ~pon_reset_n  ),		// 将外部复位信号 pon_reset_n 取反后，连接到模块的 iRst 输入端口。通常复位信号低电平有效，这里取反后，高电平表示复位
    .o1uSCE      	( w1usCE        ), 		// 模块输出的 1 微秒时钟使能信号，连接到内部信号 w1usCE，用于标记 1 微秒的时间周期
    .o10uSCE     	( w10usCE       ), 		// 模块输出的 10 微秒时钟使能信号，连接到内部信号 w10usCE
    .o50uSCE     	( w50usCE       ), 		// 模块输出的 50 微秒时钟使能信号，连接到内部信号 w50usCE
    .o500uSCE    	( w500usCE      ),	 	// 模块输出的 500 微秒时钟使能信号，连接到内部信号 w500usCE
    .o1mSCE      	( w1msCE        ), 		// 模块输出的 1 毫秒时钟使能信号，连接到内部信号 w1msCE
    .o250mSCE    	( w250msCE      ), 		// 模块输出的 250 毫秒时钟使能信号，连接到内部信号 w250msCE
    .o10mSCE     	( w10msCE       ), 		// 模块输出的 10 毫秒时钟使能信号，连接到内部信号 w10msCE
    .o20mSCE     	( w20msCE       ), 		// 模块输出的 20 毫秒时钟使能信号，连接到内部信号 w20msCE
    .o1SCE       	( w1sCE         )  		// 模块输出的 1 秒时钟使能信号，连接到内部信号 w1sCE
);
/************************************************************************************************************************************************************************/
//--------------------------------------------------------------------------------------------------------------------------------------------------
//I2C Update Start
...
.i2c1_scl	(io_I2C2_UPDATE_SCL	),	// I2C SCL（时钟）信号，连接到外部引脚 io_I2C2_UPDATE_SCL
.i2c1_sda	(io_I2C2_UPDATE_SDA	),// I2C SDA（数据）信号，连接到外部引脚 io_I2C2_UPDATE_SDA

.i2c1_irqo 	(		),		// I2C中断请求输出，此处未连接
.wbc_ufm_irq(   )     // UFM（用户闪存）中断信号（未启用，悬空）
); 
...
reg     r_dip_cpu_prsnt_override = 1'b0;// 1 = 使用拨码覆盖，0 = 使用真实去抖信号
reg     r_cpu_pwrbtn_force_n = 1'b0;    // CPU 电源按钮强制信号，低电平有效，默认不强制
//--------------------------------------------------------------------------------------------------------------------------------------------------
//For cpu_module_u1:Assume the CPU is Present CPU 模块使能与电源状态信号
...
//--------------------------------------------------------------------------------------------------------------------------------------------------
//for sync_cpu_data_low	CPU 数据同步与 PCIe 复位信号
...
//btn
// wire    db_i_pal_pwr_btn_n	  ; // pal 模块的电源按钮,低电平有效
// wire    db_i_pal_ext_rst_n	  ; //关联 pal 模块的外部复位,低电平有效
// wire    db_i_pal_bmcuid_button; //关联 pal 模块的 BMCUID 按钮,高电平有效
wire    db_i_fm_pwrbtn_out_n	  ; // 系统最终电源控制信号
//--------------------------------------------------------------------------------------------------------------------------------------------------
...
wire    db_i_p1_vr_i2c_alert_n		;	// P1电压调节模块（VR）I2C 告警（低电平有效），用于检测 P1 VR 的 I2C 接口告警
//--------------------------------------------------------------------------------------------------------------------------------------------------
...
wire    db_i_pgd_p5v              = 1'b1  ;//unused	// 5V 电源好信号，指示 5V 电源轨稳定
wire    db_i_pgd_p0_vdd_18_stby   = 1'b1  ;//04 P0 1.8V 待机电源好信号，指示 P0 1.8V 待机电源轨稳定
wire    db_i_pgd_p1_vdd_18_stby		= 1'b1  ;//05	P1 1.8V 待机电源好信号，指示 P1 1.8V 待机电源轨稳定
wire    db_i_pgd_p0_vddc				  = 1'b1  ;//06	P0 1.8V 待机电源好信号，指示 P0 1.8V 待机电源轨稳定
wire    db_i_pgd_p1_vddc          = 1'b1  ;//07 P1 1.8V 待机电源好信号，指示 P1 1.8V 待机电源轨稳定
wire    db_i_pgd_p0_vdd_11_sus		= 1'b1  ;//08	P0 11V 待机电源好信号，指示 P0 11V 待机电源轨稳定
wire    db_i_pgd_p1_vdd_11_sus    = 1'b1  ;//09 P1 11V 待机电源好信号，指示 P1 11V 待机电源轨稳定
wire    db_i_pgd_p0_vdd_core_0		= 1'b1  ;//10	P0 核心电压 0 电源好信号，指示 P0 核心电压 0 电源轨稳定
wire    db_i_pgd_p1_vdd_core_0		= 1'b1  ;//11	P1 核心电压 0 电源好信号，指示 P1 核心电压 0 电源轨稳定
wire    db_i_pgd_p0_vdd_core_1		= 1'b1  ;//12	
wire    db_i_pgd_p1_vdd_core_1		= 1'b1  ;//13
wire    db_i_pgd_p0_vdd_soc_0			= 1'b1  ;//14	P0 SOC 电压 0 电源好信号，指示 P0 SOC 电压 0 电源轨稳定
wire    db_i_pgd_p1_vdd_soc_0			= 1'b1  ;//15	P1 SOC 电压 0 电源好信号，指示 P1 SOC 电压 0 电源轨稳定
wire    db_i_pgd_p0_vddio				  = 1'b1  ;//16	P0 IO 电压电源好信号，指示 P0 IO 电压电源轨稳定
wire    db_i_pgd_p1_vddio				  = 1'b1  ;//17	P1 IO 电压电源好信号，指示 P1 IO 电压电源轨稳定
wire    db_i_pgd_pal_pvcc_hpmos   = 1'b1  ;//18 P0 HPMOS 电源好信号，指示 P0 HPMOS 电源轨稳定
...
//--------------------------------------------------------------------------------------------------------------------------------------------------
//for cup_thermtrip  CPU 热保护与电源序列从模块相关信号
//--------------------------------------------------------------------------------------------------------------------------------------------------   
...
wire    w_grp_b_pal_pvcc_hpmos_en    ;	// B 组 HPMOS 使能信号，使能 B 组 HPMOS 电源
...
wire    w_pal_p12v_cpu0_on        ;	// 平台 P12V CPU0 开启信号，使能平台 P12V CPU0 电源
wire    w_pal_p12v_cpu1_on        ;	// 平台 P12V CPU1 开启信号，使能平台 P12V CPU1 电源
wire    w_p12_en_p0_dimm          ;
wire    w_p12_en_p1_dimm          ;
...
wire    w_grp_b_pal_pvcc_hpmos_fault_det  ;	// B 组 HPMOS 故障检测，检测 B 组 HPMOS 电源故障
...
wire    w_bmc_i2c4_9548_3_rst_n		;
wire    w_bmc_i2c4_9548_4_rst_n		;

wire    w_p0_vpp_9545_1_rst_n		;	 // P0 VPP（9548 芯片）通道1复位（低电平有效
wire    w_p0_vpp_9545_2_rst_n		;	 // P0 VPP 通道2复位（低电平有效）
wire    w_p0_vpp_9545_3_rst_n		;
wire    w_p0_vpp_9545_4_rst_n		;
...
wire db_i_fm_pwrbtn_out_n_r   ; //系统最终电源控制信号
...
// wire    w_p12v_slot_0_on_r;		// P12V 槽位 0 使能信号（同步后），控制 P12V 槽位 0 的使能（同步后）
...
wire w_pal_3v3_pgd1_r_sw       ; //u7 平台级 3.3V 电源好 1 信号（同步后，Switch 相关），平台级 3.3V 电源轨 1 的稳定状态（Switch 场景下）
wire w_pal_3v3_pgd2_r_sw       ; //u7
wire w_pal_3v3_pgd3_r_sw       ; //u7
wire w_pal_3v3_pgd4_r_sw       ; //u7
wire w_pal_3v3_pgd5_r_sw       ; //u7
wire w_u3_nc7_sw               ; //u7 U3 引脚 NC7 信号（Switch 相关），U3 引脚 NC7 的状态（Switch 场景下，实际可能未连接，用于占位）
...
wire w_board_id0_sw            ;  //u6	SW 系列板卡 ID0 检测
wire w_board_id1_sw            ;  //u6	SW 系列板卡 ID1 检测
...
wire w_ct_p1v25_sw0_pg_sw      ; //u41	CT PLV25 SW0 电源好信号
wire w_ct_p1v25_sw1_pg_sw      ; //u41	CT PLV25 SW1 电源好信号
wire w_p0v8_sw0_pwrgd_sw       ; //u41	P0V8 SW0 电源好信号
wire w_p0v8_sw1_pwrgd_sw       ; //u41	P0V8 SW1 电源好信号
wire w_slot1_wake_n_sw         ; //u41	slot1 唤醒信号（低电平有效）
...
wire w_slot1_prsnt_n_zt        ;//u7	ZT 系列 slot1 存在检测
wire w_slot2_prsnt_n_zt        ;//u7	ZT 系列 slot2 存在检测
...
wire w_mcio1_prsnt_n_zt        ;//u8    ZT 系列 mcio1 存在检测
wire w_mcio2_prsnt_n_zt        ;//u8    ZT 系列 mcio2 存在检测
...
wire [5:0]pvti_zt_count;	// PVTI ZT 计数（6位），用于 PVTI 相关的计数逻辑
wire w_pwr_on_dly2s;		// 上电延迟 2 秒信号，控制上电过程中的 2 秒延迟
wire w_pwr_on_dly1s5;  		// 上电延迟 2 秒信号，控制上电过程中的 2 秒延迟
wire [2:0]pvti_sw_u2_count1;	// PVTI SW U2 计数（3位），用于 PVTI SW U2 相关的计数
...
wire    w_u19_nc2_zt2              ;//u19   U19 引脚 NC2（未连接）检测（ZT2 系列）
wire    w_u19_nc3_zt2              ;//u19   U19 引脚 NC3（未连接）检测（ZT2 系列）
wire    w_u19_nc4_zt2              ;//u19   
wire    w_u19_nc5_zt2              ;//u19   
wire    w_u19_nc6_zt2              ;//u19   
wire    w_u19_nc7_zt2              ;//u19   

reg      r_board_id0_zt2     ;	// 锁存 ZT2 板卡 ID0 状态
reg      r_board_id1_zt2     ;	// 锁存 ZT2 板卡 ID1 状态
reg      r_board_id2_zt2     ;
reg      r_board_id3_zt2     ;
reg      r_board_id4_zt2     ;
reg      r_board_id5_zt2     ;
reg      r_board_id6_zt2     ;
reg      r_board_id7_zt2     ;
reg      r_pcb_version2_zt2  ;	// 锁存 ZT2 PCB 版本 2 状态
reg      r_pcb_version1_zt2  ;	// 锁存 ZT2 PCB 版本 1 状态
reg      r_pcb_version0_zt2  ;
reg      r_pca_version2_zt2  ;
reg      r_pca_version1_zt2  ;
reg      r_pca_version0_zt2  ;
reg      r_mcio9_prsnt_n_zt2 ;	// 锁存 ZT2 mcio9 存在状态（低电平有效）
...
//SIGNAL DEBOUNCE
//------------------------------------------------------------------------------------------------//
PGM_DEBOUNCE #(.SIGCNT(3),	 // 配置输入信号数量为 3 路（对应电源按钮、复位按钮、BMC ID 按钮）
 .NBITS(2'b10),	// 配置内部计数器位宽为 2 位（可计数范围 0~3，满足 3 路信号消抖的时序控制）
 .ENABLE(1'b1)	// 使能消抖功能（高电平有效）
 ) db_inst_pwr_btn(	//按键消抖模块 db_inst_pwr_btn
  .clk(clk_50m),
  .rst(~pon_reset_n),	// 复位信号（取反后接入，因为模块内复位为高电平有效，而 pwr_reset_n 是低电平有效）
  .timer_tick(t32ms_tick),	
  .din({	 		// 输入信号拼接，共 3 路
    i_FM_PWRBTN_OUT_N_R,
    i_FM_RSTBTN_OUT_N_R
		//i_PAL_PWR_BTN_N     ,//01	电源按钮
                // i_PAL_BUTTOPN_RST_N ,//02	电源按钮
                // i_PAL_BMCUID_BUTTON //03	BMC ID 按钮
	   }),             
  .dout({			 // 输出信号拼接，共 3 路
    db_i_fm_pwrbtn_out_n_r,
    db_i_fm_rstbtn_out_n_r
		//db_i_pwr_btn_cpld_n_r ,//01	消抖后电源按钮
                //  db_i_pal_ext_rst_n  ,//02	消抖后复位信号
                // db_i_pal_bmcuid_button  //03	消抖后 BMC ID 按钮
       }) 
);

//for test
// reg [4:0] db_i_fm_pwrbtn_out_n_cnt;

// always @(posedge clk_50m or negedge pon_reset_n) begin
//     if(!pon_reset_n)
//         db_i_fm_pwrbtn_out_n_cnt <= 5'd0;
//     else if(db_i_fm_pwrbtn_out_n_cnt == 5'd15)
//         db_i_fm_pwrbtn_out_n_cnt <= db_i_fm_pwrbtn_out_n_cnt;
//     else if(t16us_tick)
//         db_i_fm_pwrbtn_out_n_cnt <= db_i_fm_pwrbtn_out_n_cnt + 1'b1;
// end

// always @(posedge clk_50m or negedge pon_reset_n) begin
//     if(!pon_reset_n)
//         db_i_fm_pwrbtn_out_n_r <= 1'b1;
//     else if(db_i_fm_pwrbtn_out_n_cnt == 5'd7)
//         db_i_fm_pwrbtn_out_n_r <= 1'b0;
//     else if(db_i_fm_pwrbtn_out_n_cnt == 5'd15)
//         db_i_fm_pwrbtn_out_n_r <= 1'b1;
// end

//--------------------------------------------------------------------------------------------------------------------------------------------------
//VR OCP Signal DEBOUNCE	VR 过流保护消抖模块
//--------------------------------------------------------------------------------------------------------------------------------------------------
PGM_DEBOUNCE #(.SIGCNT(6),	// 配置输入信号数量为 6 路（对应 P0/P1 各核心、IO 电压的过流保护信号）
 .NBITS(2'b10),
 .ENABLE(1'b1)
 ) db_vr_ocp_low (
  .clk(clk_50m),
  ...
  .din({
			i_P0_VDD_CORE_0_OCP_N_R			,//01	P0 核心电压 0 过流
			i_PAL_P0_VDD_CORE_1_OCP_N		,//02	P0 核心电压 1 过流
			i_P0_VDDIO_OCP_N			 	,//03	P0 IO 电压过流
      i_P1_VDD_CORE_0_OCP_N_R			,//04
			i_PAL_P1_VDD_CORE_1_OCP_N		,//05
			i_P1_VDDIO_OCP_N			 	  //06                 
  }),             
  .dout({
			db_i_p0_vdd_core_0_ocp_n_r		,//01	消抖后 P0 核心电压 0 过流
...
//-------------------------------------------------------------------------------------------------
// for PSU Signal DEBOUNCE        8 Signal 对电源供应单元（PSU）相关信号进行消抖处理
// ------------------------------------------------------------------------------------------------
// PGM_DEBOUNCE #(.SIGCNT(10),	// 配置输入信号数量为 10 路，对应 10 个 `PSU` 相关信号
//  .NBITS(2'b10),		// 配置内部计数器位宽为 2 位，满足 10 路信号消抖的时序控制
//  .ENABLE(1'b1)		// 使能消抖功能（高电平有效
//  ) db_inst_psu (
//   .clk(clk_50m),
//   .rst(~pon_reset_n),
//   .timer_tick(t64ms_tick),
//   .din({
//                  i_PS1_PRSNT                    ,   //01	PS1 存在检测信号，检测 PS1 是否存在
//                  i_PS1_DCOK_N                  ,   //02		PS1 直流正常信号（低电平有效），指示 PS1 直流输出正常
//                  i_PS1_SMB_ALERT            ,   //03		PS1 SMB 告警信号，PS1 通过 SMB 总线产生的告警
//                  i_PS1_ACFAIL_N              ,   //04		PS1 交流故障信号（低电平有效），指示 PS1 交流输入故障
//                  i_PS2_PRSNT                    ,   //05
//                  i_PS2_DCOK_N                  ,   //06
//                  i_PS2_SMB_ALERT            ,   //07
//                  i_PS2_ACFAIL_N              ,   //08
//                  w_ps3_prsnt                    ,   //09	PS3 存在检测信号，检测 PS3 是否存在
//                  w_ps4_prsnt                         //10	
//   }),
//   .dout({
//                  db_i_ps1_prsnt              ,   //01
...
//   .timer_tick	(1'b1),
//   .din({
//              i_P1V8_STBY_PG				,//01	P1V8 待机电源好信号，P1V8 待机电源轨稳定指示
//              i_PWRGD_P3V3_STBY			        ,//02	3.3V 待机电源好信号，3.3V 待机电源轨稳定指示
//              //i_PG_P5V_STBY				        ,//03	5V 待机电源好信号，5V 待机电源轨稳定指示
//              i_PGD_P0_VDD_18_STBY                   ,//04	P0 1.8V 待机电源好信号，P0 1.8V 待机电源轨稳定指示
//              i_PGD_P1_VDD_18_STBY		        ,//05	P1 1.8V 待机电源好信号，P1 1.8V 待机电源轨稳定指示
//              i_P0_PWRGD_VDD_3V3_STBY				        ,//06 	P0 VDDC 电源好信号，P0 VDDC 电源轨稳定指示
//              i_P1_PWRGD_VDD_3V3_STBY                                 ,//07 
//              i_PGD_P0_VDD_11_SUS		        ,//08	P0 11V 待机电源好信号，P0 11V 待机电源轨稳定指示
//              i_PGD_P1_VDD_11_SUS                     ,//09
//              i_PGD_P0_VDD_CORE_0		        ,//10	P0 核心电压 0 电源好信号，P0 核心电压 0 电源轨稳定指示
//              i_PGD_P1_VDD_CORE_0		        ,//11
//              i_PGD_P0_VDD_CORE_1		        ,//12
//              i_PGD_P1_VDD_CORE_1		        ,//13
//              i_PGD_P0_VDD_SOC_0			,//14	P0 SOC 电压 0 电源好信号，P0 SOC 电压 0 电源轨稳定指示
//              i_PGD_P1_VDD_SOC_0			,//15
//              i_PGD_P0_VDDIO				,//16	P0 SOC 电压 0 电源好信号，P0 SOC 电压 0 电源轨稳定指示
//              i_PGD_P1_VDDIO				,//17
//              i_PG_PVCC_HPMOS_R     ,//18  高压 MOSFET 电源好信号，高压 MOSFET 电源轨稳定指示
//              //i_PGD_P3V3_STBY_B          ,//18	3.3V 待机电源好反相信号，3.3V 待机电源轨稳定的反相指示
//              //i_PGD_P1V2_STBY            ,//19	1.2v 待机电源好信号，1.2v 待机电源轨稳定指示
//              //i_PGD_P5V                  ,//20	5V 电源好信号，5V 主电源轨稳定指示
//              i_PG_P1V0_STBY_M2_R		 //		1.0v 待机 M2 电源好信号，1.0v 待机 M2 电源轨稳定指示
// 	 }),             
//   .dout({
...
PGM_DEBOUNCE #(.SIGCNT(3), .NBITS (2'b11), .ENABLE(1'b1)) db_inst_amd_cpu_prsnt(   
  ...
  .rst(~pon_reset_n),
  .din({  
                (i_P0_PRSNT_N ) ,	//01 P0 存在与 CPU 模块 P0 存在的与运算信号（低电平有效），P0 及对应 CPU 模块存在的复合指示
                (i_P1_PRSNT_N ) ,  //02 P1 存在与 CPU 模块 P1 存在的与运算信号（低电平有效），P1 及对应 CPU 模块存在的复合指示
                i_P0_SPD_HOST_CTRL_N				        //03 P0 SPD 主机控制信号（低电平有效），P0 SPD 主机控制指示
	  }),  
  .dout({	  
                db_cpu_prsnt_n_db[0],   //01	稳定的 P0 侧设备存在指示
                db_cpu_prsnt_n_db[1],   //02	稳定的 P1 侧设备存在指示
                db_i_p0_spd_host_ctrl_n            //03	稳定的 P0 SPD 主机控制指示
		})
);

// 最终选择逻辑：若启用覆盖，则取 r_dip_cpu_prsnt_n；否则使用去抖结果 db_cpu_prsnt_n_db
// assign db_cpu_prsnt_n = 2'b11;
...
//   .timer_tick(t64ms_tick),
//   .din({
//              i_PGD_P12V_DROOP            , //1	F12V 掉电信号，F12V 电压跌落指示
//              i_PGD_P12V_STBY_DROOP         //2	F12V 待机掉电信号，F12V 待机电压跌落指示
//   }),
//   .dout({
...
// 实例化边缘检测模块，用于检测 F12V 掉电信号的下降沿
// Edge_Detect Edge_Detect_U1(    //2023-3-9 add 
//     .i_clk               (clk_50m),        
//     .i_rst_n             (pon_reset_n),       
//     .i_signal            (i_PGD_P12V_DROOP),//i_PGD_P12V_DROOP  w_PWRGD_P12VF12V 掉电原始信号，待检测下降沿的目标信号
    
//     .o_signal_pos        (),
//     .o_signal_neg        (w_pgd_p12v_droop_neg),//信号下降沿检测结果，赋值给 w_pgd_p12v_droop_neg
//     .o_signal_invert     ()
// );
...
// 	end
// 	else if(w_pgd_p12v_droop_neg) begin	// 正常工作逻辑：当检测到 F12V 掉电下降沿时
//         r_p12v_discharge_r <= 1'b1;		// 放电寄存器置为高电平，触发放电控制
// 	end
// end
// assign  w_p12v_discharge_r = r_p12v_discharge_r;	// 组合逻辑：将寄存器状态赋值给输出线网，对外提供放电控制信号
...
assign  mcpld_to_scpld_p2s_data[32]        = w_p12_en_p1_dimm        ;
assign  mcpld_to_scpld_p2s_data[31]        = w_p12_en_p0_dimm        ;
...
assign  mcpld_to_scpld_p2s_data[21]        = db_i_fm_rstbtn_out_n_r ;
assign  mcpld_to_scpld_p2s_data[20]        = 1'b0         ;
	...
	begin
     ...
     // 正常工作状态：校验SGPIO接收数据的帧头（判断数据是否有效）
        ...
        // 帧头校验通过：提取有效数据（去除帧头）
			...
			begin // 否则，scpld_to_mcpld_data_filter 保持原值
				scpld_to_mcpld_data_filter <= scpld_to_mcpld_data_filter;// 保持原滤波数据（避免无效数据覆盖）
				scpld_sgpio_fail <=1'b1;// 置 scpld_sgpio_fail 为 1，标识 SGPIO 故障
			end

end
//S CPLD ---> M CPLD 从 CPLD（S CPLD）接收串行数据并转为并行数据的核心
s2p_master #(.NBIT(200)) inst_scpld_to_mcpld_s2p(//96NBIT表示数据位宽（200）位，通过修改NBIT即可适配不同位宽的串行通信需求，无需修改模块内部逻辑，同前面的PGM_DEBOUNCE的SIGCNT参数类似
  .clk  (clk_50m					), //in
  .rst  (~pon_reset_n				), //in
  .tick (t1us_tick					), //in
  .si   (i_CPLD_SGPIO0_MISO_R		), //in   //SGPIO_MISO  Serial Signal input 输入：SGPIO MISO 串行输入信号
  .po   (scpld_to_mcpld_s2p_data	), //out  //Parallel Signal output 输出：并行输出信号
  .sld_n(w_cpld_sgpio0_ld_n_r	), //out  //SGPIO_LOAD SGPIO 装载信号（低电平有效）
  .sclk (w_cpld_sgpio0_clk_r 	)  //out  //SGPIO_CLK SGPIO 时钟信号
);

//M CPLD  ---> S CPLD 并行转串行从模块，将CPLD内部的并行数据转为串行数据，发送给主板，实现CPLD到主板的反向数据回传 M (主）CPLD 发送并行数据到 S CPLD 的核心
p2s_slave #(.NBIT(200)) inst_mcpld_to_scpld_p2s(//96
	.clk  (clk_50m					    ),//in
	.rst  (~pon_reset_n				    ),//in
	.pi   (mcpld_to_scpld_p2s_data	    ),//in   //Parallel Signal input 并行输入信号
	.so   (w_cpld_sgpio0_mosi_r 	),//out  //SGPIO_MOSI Serial Signal output SGPIO MOSI 串行输出信号
	.sld_n(w_cpld_sgpio0_ld_n_r		),//in   //SGPIO_LOAD SGPIO 装载信号（低电平有效）
	.sclk (w_cpld_sgpio0_clk_r		) //in   //SGPIO_CLK SGPIO 时钟信号
);
// 双向通信流程
...
//MB CPLD ---> CMU CPLD （从 MB CPLD 到 CMU CPLD 的信号赋值，将 mbcpld_to_cmucpld_p2s_data 各位赋值为对应信号或固定值）
...
assign mbcpld_to_cmucpld_p2s_data[18]    = w_rfu_bit3                     ;// 保留位3
...
always@(posedge clk_50m or negedge pon_reset_n)
	begin
        ...
        // 帧头校验通过：提取4~23位（共20位）有效数据到滤波寄存器
			...
			begin// 否则，cmu_to_mb_data_filter 保持原值
...
power_button #(.BL_MODE(1'b0)) power_button_inst_local (
  .clk                   (clk_50m),
  ...
  .st_steady_pwrok       (1'b1),                  // 置 1 以允许 force_off -> THRMTRIP 生效
...
assign  w_pal_pwrbtn_n_r = db_i_fm_pwrbtn_out_n_r & ~w_bmc_sbtn_poweron ;  //o_FM_CPLD_PWRvBTN_OUT_N    cpld TO pch
// assign  w_pal_pwrbtn_n_r = w_pwrbtn_to_pch_n ;
...
cpu_module cpu_module_u1	(
    ...
    .i_p0_pwr_btn_n		(~w_pal_pwrbtn_n_r				),	// 输入：CPU 0 电源按钮信号（低电平有效，取反后输入模块）
...
//把 pwrseq_master / pwrseq_slave 的输出先接到 *_orig（不改变外部端口声明） ----------
//作用：默认全部上电-for test

/* declare orig wires for master outputs */
wire [5:0]  w_power_seq_sm_orig;
wire        w_power_fault_orig;
wire        w_stby_failure_detected_orig;
wire        w_dc_failure_detected_orig;
wire        w_rt_failure_detected_orig;
wire        w_cpld_latch_sys_off_orig;
wire        w_turn_on_wait_orig;
wire        w_fault_clear_orig;
wire        w_cmu_fault_clear_orig;
wire        w_dc_on_wait_complete_orig;
wire        w_rt_critical_fail_store_orig;

/* declare orig wires for slave <-> master feedback and slave outputs */
wire        w_pgd_so_far_orig;
wire        w_s5dev_aux_pwren_request_orig;
wire        w_s5dev_aux_pwrdis_request_orig;
wire        w_any_pwr_fault_det_orig;
wire        w_any_lim_recov_fault_orig;
wire        w_any_non_recov_fault_orig;
wire        w_any_aux_vrm_fault_orig;
wire        w_any_recov_fault_orig;

wire        w_p5v_stby_en_orig;
wire        w_p5v_stby_usb_en_orig;
wire        w_grp_b_p0_33_s5_en_orig;
wire        w_grp_b_p1_33_s5_en_orig;
wire        w_grp_b_p0_18_s5_en_orig;
wire        w_grp_b_p1_18_s5_en_orig;
wire        w_p12_en_orig;
wire        w_p5v_en_orig;
wire        w_grp_c_p0_vdd11_en_orig;
wire        w_grp_c_p1_vdd11_en_orig;
wire        w_grp_d_p0_vddio_en_orig;
wire        w_grp_d_p1_vddio_en_orig;
wire        w_grp_d_p0_soc_en_orig;
wire        w_grp_d_p1_soc_en_orig;
wire        w_grp_d_p0_vddcore0_en_orig;
wire        w_grp_d_p1_vddcore0_en_orig;
wire        w_grp_d_p0_vddcore1_en_orig;
wire        w_grp_d_p1_vddcore1_en_orig;




wire        w_cpu_pwr_good_orig;
wire [1:0]  o_cpu_pwrok_orig;
wire        w_rsmrst_n_orig;
wire        w_pwr_btn;
   ...
   pwrseq_master  pwrseq_master_inst (
    ...
    .p0_pwrbtn_n				(~w_pal_pwrbtn_n_r			),	// 输入：P0 CPU 电源按钮信号（低电平有效）
    ...
    .pwr_btn_long     (pch_thrmtrip      ),	// 输入：电源按钮长按信号（来自按钮去抖模块）
                                                                          // 功能：检测到电源按钮被长按（如超过 4 秒），触发强制关机逻辑
    // ==============================================
    ...
    .any_lim_recov_fault		(1'b0		),	// 输入：任意有限恢复故障信号（来自 pwrweq_slave）
                                                                          // 功能：轻微故障（如电压波动），可通过重试恢复
    .any_non_recov_fault		(1'b0		),	// 输入：任意非恢复故障信号（来自 pwrweq_slave）
...
// 注：PG信号为高电平有效，表示该电源通道电压/电流达到额定值，稳定可用；低电平表示异常
  ...
  .grp_b_pal_pvcc_hpmos_pg        (db_i_pgd_pal_pvcc_hpmos  ), // 输入：PAL（平台辅助逻辑）PVCC HPMOS电源PG信号（去抖后，如PAL芯片供电）  
...
//to Power Controller Enable Pin：向电源控制器输出的“电源使能信号”（高电平有效，1=使能供电，0=断开供电）
  ...
  .grp_b_pal_pvcc_hpmos_en        (w_grp_b_pal_pvcc_hpmos_en),	// 输出：PAL（平台辅助逻辑）PVCC HPMOS电源使能信号
                                                              // 控制对象：PAL芯片的PVCC供电                                                            
  ...
  .pal_p12v_cpu0_on         (w_pal_p12v_cpu0_on       ),  // 注释备用：PAL芯片控制P0 CPU 12V电源使能信号
  .pal_p12v_cpu1_on         (w_pal_p12v_cpu1_on       ),  // 注释备用：PAL芯片控制P1 CPU 12V电源使能信号
  .p12_en_p0_dimm_1         (w_p12_en_p0_dimm         ),  // 注释备用：P0内存12V第1通道使能信号
  .p12_en_p1_dimm_1         (w_p12_en_p1_dimm         ),  // 注释备用：P1内存12V第1通道使能信号
...
//fault detect：各电源通道的故障检测信号（高电平有效，1=检测到故障，0=正常）
  ...
  .grp_b_p1_33_s5_fault_det	        (w_grp_b_p1_33_s5_fault_det	),	// 输出：P1 CPU组（grp_b）3.3V S5电源故障检测信号
  ...
  .grp_b_p1_18_s5_fault_det	        (w_grp_b_p1_18_s5_fault_det	),	// 输出：P1 CPU组（grp_b）1.8V S5电源故障检测信号
                                                              // 故障类型：P1 CPU 1.8V S5电压异常、休眠供电故障
  .grp_b_pal_pvcc_hpmos_fault_det  (w_grp_b_pal_pvcc_hpmos_fault_det),	// 输出：PAL PVCC HPMOS电源故障检测信号
                                                              // 故障类型：PAL PVCC HPMOS电压异常、VRM故障
...
//from CPU
  .i_cpu_pwrok					(w_cpu_pwrok				),	//from CPU PWROK 输入：CPU 电源好确认信号（来自 CPU 内部 PWROK 引脚）
...
// // ---------- 新增：覆盖 assign（测试专用：强制全部上电） ----------
// /* 测试开关：1=强制全部上电（跳过时序/故障），0=恢复由 pwrseq 模块产生的原始行为 */
// reg force_all_power_on;
// always @(posedge clk_50m or negedge pon_reset_n) begin
//     if (~pon_reset_n) begin
//         force_all_power_on <= 1'b0;
//     end
//     else if (t128ms_tick) begin
//         // 在第一个 1s tick 到来时置位，使能强制全部上电
//         force_all_power_on <= 1'b1;
//     end
//     else begin
//         force_all_power_on <= force_all_power_on;
//     end
// end


// // master outputs override
// assign w_power_seq_sm                = force_all_power_on ? SM_STEADY_PWROK : w_power_seq_sm_orig;
// assign w_power_fault                 = force_all_power_on ? 1'b0 : w_power_fault_orig;
// assign w_stby_failure_detected       = force_all_power_on ? 1'b0 : w_stby_failure_detected_orig;
// assign w_dc_failure_detected         = force_all_power_on ? 1'b0 : w_dc_failure_detected_orig;
// assign w_rt_failure_detected         = force_all_power_on ? 1'b0 : w_rt_failure_detected_orig;
// assign w_cpld_latch_sys_off          = force_all_power_on ? 1'b0 : w_cpld_latch_sys_off_orig;
// assign w_turn_on_wait                = force_all_power_on ? 1'b0 : w_turn_on_wait_orig;
// assign w_fault_clear                 = force_all_power_on ? 1'b1 : w_fault_clear_orig;
// assign w_cmu_fault_clear             = force_all_power_on ? 1'b1 : w_cmu_fault_clear_orig;
// assign w_dc_on_wait_complete         = force_all_power_on ? 1'b1 : w_dc_on_wait_complete_orig;
// assign w_rt_critical_fail_store      = force_all_power_on ? 1'b0 : w_rt_critical_fail_store_orig;

// // slave feedback override (to master)
// assign w_pgd_so_far                  = force_all_power_on ? 1'b1 : w_pgd_so_far_orig;
// assign w_s5dev_aux_pwren_request     = force_all_power_on ? 1'b0 : w_s5dev_aux_pwren_request_orig;
// assign w_s5dev_aux_pwrdis_request    = force_all_power_on ? 1'b0 : w_s5dev_aux_pwrdis_request_orig;
// assign w_any_pwr_fault_det           = force_all_power_on ? 1'b0 : w_any_pwr_fault_det_orig;
// assign w_any_lim_recov_fault         = force_all_power_on ? 1'b0 : w_any_lim_recov_fault_orig;
// assign w_any_non_recov_fault         = force_all_power_on ? 1'b0 : w_any_non_recov_fault_orig;
// assign w_any_aux_vrm_fault           = force_all_power_on ? 1'b0 : w_any_aux_vrm_fault_orig;
// assign w_any_recov_fault             = force_all_power_on ? 1'b0 : w_any_recov_fault_orig;

// // slave outputs (power enables) override -> 强制所有使能为 1（上电）
// assign w_p5v_stby_en                 = force_all_power_on ? 1'b1 : w_p5v_stby_en_orig;
// assign w_p5v_stby_usb_en             = force_all_power_on ? 1'b1 : w_p5v_stby_usb_en_orig;
// assign w_grp_b_p0_33_s5_en           = force_all_power_on ? 1'b1 : w_grp_b_p0_33_s5_en_orig;
// assign w_grp_b_p1_33_s5_en           = force_all_power_on ? 1'b1 : w_grp_b_p1_33_s5_en_orig;
// assign w_grp_b_p0_18_s5_en           = force_all_power_on ? 1'b1 : w_grp_b_p0_18_s5_en_orig;
// assign w_grp_b_p1_18_s5_en           = force_all_power_on ? 1'b1 : w_grp_b_p1_18_s5_en_orig;
// assign w_p12_en                      = force_all_power_on ? 1'b1 : w_p12_en_orig;
// assign w_p5v_en                      = force_all_power_on ? 1'b1 : w_p5v_en_orig;
// assign w_grp_c_p0_vdd11_en           = force_all_power_on ? 1'b1 : w_grp_c_p0_vdd11_en_orig;
// assign w_grp_c_p1_vdd11_en           = force_all_power_on ? 1'b1 : w_grp_c_p1_vdd11_en_orig;
// assign w_grp_d_p0_vddio_en           = force_all_power_on ? 1'b1 : w_grp_d_p0_vddio_en_orig;
// assign w_grp_d_p1_vddio_en           = force_all_power_on ? 1'b1 : w_grp_d_p1_vddio_en_orig;
// assign w_grp_d_p0_soc_en             = force_all_power_on ? 1'b1 : w_grp_d_p0_soc_en_orig;
// assign w_grp_d_p1_soc_en             = force_all_power_on ? 1'b1 : w_grp_d_p1_soc_en_orig;
// assign w_grp_d_p0_vddcore0_en        = force_all_power_on ? 1'b1 : w_grp_d_p0_vddcore0_en_orig;
// assign w_grp_d_p1_vddcore0_en        = force_all_power_on ? 1'b1 : w_grp_d_p1_vddcore0_en_orig;
// assign w_grp_d_p0_vddcore1_en        = force_all_power_on ? 1'b1 : w_grp_d_p0_vddcore1_en_orig;
// assign w_grp_d_p1_vddcore1_en        = force_all_power_on ? 1'b1 : w_grp_d_p1_vddcore1_en_orig;

// // CPU / VR related overrides
// assign w_cpu_pwr_good                = force_all_power_on ? 1'b1 : w_cpu_pwr_good_orig;
// assign o_cpu_pwrok                   = force_all_power_on ? {`NUM_CPU{1'b1}} : o_cpu_pwrok_orig; // all CPU PWROK = 1
// assign w_rsmrst_n                    = force_all_power_on ? 1'b1 : w_rsmrst_n_orig;
...
// 赋值所有电源好信号：当所有待机电源和主电源都好时，该信号为真							 
...
fault_detectB_chklive #(.NUMBER_OF_VRM(1))//用于适配不同数量的 VRM 监控场景（如单路 VRM、多路 VRM）,即当前实例化的 fault_detect_chklive 模块仅监控1 路 VRM
...
UART_MASTER #(.NBIT_IN(16), .NBIT_OUT(16), .BPS_COUNT_NUM(48), .START_COUNT_NUM(24)) uart_master_u2 (
    ...
    .mcio_cable_id1  (w_P0_MCIOP0C_CB_ID1_R     )  ,  // P0C 通道线缆 ID1
    .error_flag          (  )                          
);
//UART 模块核心作用：
...
//BIOS_CPLD_IIC
//------------------------------------------------------------------------------------------------//
//该部分代码聚焦于 服务器 BIOS 与 CPLD 之间的 I2C 接口通信，核心模块 bios_cpld_i2c_ram 承担 “硬件状态缓存”“配置指令存储”“I2C 数据收发” 
//三大功能，是 BIOS 实现 “远程硬件监控与控制” 的关键桥梁
// I2C 调试缓存信号：存储 CPLD 采集的硬件状态，地址范围 0x1000-0x1006，BIOS 可通过 I2C 读取这些地址获取状态
wire [7:0]   w_bios_debug_ram_1000; // 调试缓存 0x1000：存储 P0 MCIOG3A 槽硬件状态（如线缆连接状态）
wire [7:0]   w_bios_debug_ram_1001; // 调试缓存 0x1001：存储 P0 MCIOG3C 槽硬件状态
wire [7:0]   w_bios_debug_ram_1002; // 调试缓存 0x1002：存储 P0 MCIOP0A 槽硬件状态
wire [7:0]   w_bios_debug_ram_1003; // 调试缓存 0x1003：存储 P0 MCIOP0C 槽硬件状态
wire [7:0]   w_bios_debug_ram_1004; // 调试缓存 0x1004：存储 P0 MCIOP1A 槽硬件状态
wire [7:0]   w_bios_debug_ram_1005; // 调试缓存 0x1005：存储 P0 MCIOP1C 槽硬件状态
wire [7:0]   w_bios_debug_ram_1006; // 调试缓存 0x1006：测试用寄存器（验证 I2C 通信是否正常，如写 0x55 读回 0x55 则通信正常）


//bios_cpld_i2c_ram 是 BIOS-CPLD I2C 通信的核心，集成 “数据缓存” 与 “I2C 物理层驱动”，实现 “BIOS 读状态、写配置” 的完整交互。
bios_cpld_i2c_ram #(
.DLY_LEN       (16)   //50MHz,330ns  // I2C 通信延迟长度：50MHz 时钟下，16 个时钟周期对应延迟=16*(1/50MHz)=320ns（匹配 I2C 标准时序）
...
.io_sda			(io_P0_CPLD_SDA	),  // 双向：I2C 数据信号（BIOS 与 CPLD 之间的双向数据传输，SDA 需开漏输出）
...
///////////////////////////////////pcie dync alloc end 0x1000-0x1005/////////////////////////
	//test 0x1006
// 调试缓存输出：将硬件状态按固定地址缓存，BIOS 通过 I2C 读取这些地址获取状态
// 地址对应关系：0x1000→w_bios_debug_ram_1000，0x1001→w_bios_debug_ram_1001，...，0x1006→w_bios_debug_ram_1006
...
//----------------------------------------------------------------------------------//
//delay 扩展槽上电延迟控制，确保前一个槽位电源稳定后再启动下一个槽位，避免电源冲击
//----------------------------------------------------------------------------------//
...
// 1. CPU0 扩展槽 0 上电延迟：输入信号稳定后延迟 10ms（10个1ms周期）再输出
edge_delay #(
    .CNTR_NBITS(6),    // 计数器位数：6位可计数 0-63，满足10ms延迟需求（10*1ms=10ms）
    .DELAY_MODE(1'b0)  // 延迟模式：0=仅对输入信号的上升沿延迟（避免下降沿误触发）
) edge_delay_slot_0_on_dly_10ms (   
    .clk         (clk_50m),          // 输入：系统时钟（50MHz，延迟计数的时序基准）
    .reset       (~pon_reset_n),     // 输入：复位信号（低电平有效，复位时延迟计数器清零）
    .cnt_size    (6'd10),            // 计数阈值：10个计数步长（每个步长=1ms，总延迟=10*1ms=10ms）
    .cnt_step    (t1ms_tick),        // 计数步长：1ms 时钟使能（每1ms触发一次计数）
    .signal_in   (db_i_p0_slp_s5_n), // 输入：触发信号（CPU0 S5休眠状态解除，1=S5解除，可上电）
                                     // 注：2024-4-10 修改为 db_i_p0_slp_s5_n，原信号为 w_PWRGD_P12V，优化为“休眠解除”触发
    .delay_output(w_slot_0_on_dly_10ms) // 输出：延迟后的信号（10ms后输出高电平，触发扩展槽0上电）
);

// 2. CPU0 扩展槽 1 上电延迟：基于槽0延迟信号，再延迟10ms，实现“槽0→槽1”顺序上电
edge_delay #(.CNTR_NBITS(6), .DELAY_MODE(1'b0)) edge_delay_slot_1_on_dly_10ms (   
    .clk         (clk_50m),
    .reset       (~pon_reset_n),
    .cnt_size    (6'd10),            // 总延迟=10ms
    .cnt_step    (t1ms_tick),
    .signal_in   (w_slot_0_on_dly_10ms), // 输入：槽0延迟后的上电信号（槽0稳定后触发槽1）
    .delay_output(w_slot_1_on_dly_10ms)  // 输出：槽1上电触发信号
);

// 3. CPU0 扩展槽 2 上电延迟：基于槽1延迟信号，再延迟10ms，实现“槽1→槽2”顺序上电
edge_delay #(.CNTR_NBITS(6), .DELAY_MODE(1'b0)) edge_delay_slot_2_on_dly_10ms (   
    .clk         (clk_50m),
    .reset       (~pon_reset_n),
    .cnt_size    (6'd10),
    .cnt_step    (t1ms_tick),
    .signal_in   (w_slot_1_on_dly_10ms), // 输入：槽1延迟后的上电信号
    .delay_output(w_slot_2_on_dly_10ms)  // 输出：槽2上电触发信号
);
...
//--------------------------------------------------------------------------------------------------------------------------------------------------
//RTC Sensor SW
//清除 CMOS 配置（RTC 实时时钟芯片的配置数据），用于恢复 BIOS 默认设置（如密码遗忘、启动配置错误）
clear_cmos clear_cmos_int(
 .i_clk					(clk_50m			),  // 输入：系统时钟（50MHz，模块时序基准）
 .i_rst_n				(~w_st_steady_pwrok & pon_reset_n	),  // 输入：复位信号（仅当“系统未稳定”且“全局复位有效”时复位）
                                                  // 逻辑：w_st_steady_pwrok=0（系统未进入稳定状态），pon_reset_n=1（全局复位解除）
 .i_clr_cmos_flg		        (w_clr_cmos_flg		),  // 输入：CMOS 清除触发信号（来自 BMC I2C 寄存器 0x03A0 bit4，1=触发清除）
 .i_t64ms_tick			(t1s_tick			),  // 输入：时钟使能（1秒周期，控制清除动作的持续时间）
                                                  // 注：原设计为 t64ms_tick，修改为 t1s_tick，延长清除时间确保配置完全重置
 .o_clr_cmos_done		(w_clr_cmos_done	),  // 输出：CMOS 清除完成标志（1=清除完成，反馈给 BMC）
 .o_clr_cmos_done_rst	(w_clr_cmos_done_rst	)  // 输出：CMOS 清除复位信号（1=触发 RTC 芯片复位，清除配置数据）
);
// 触发来源：BMC 通过 I2C 写入寄存器 0x03A0 bit4（w_clr_cmos_flg=1），远程触发 CMOS 清除 —— 无需现场操作服务器，适用于远程运维；
// 清除逻辑：o_clr_cmos_done_rst=1 时，向 RTC 芯片发送复位信号，清除其存储的 BIOS 配置（如启动顺序、硬件参数），恢复默认设置；
// 安全保护：仅当系统未进入稳定状态（w_st_steady_pwrok=0）时允许清除，避免运行中清除导致系统崩溃。

// clear_nmi：非屏蔽中断（NMI）控制，用于触发 CPU 紧急中断（如硬件故障时的内存dump、故障诊断）
 clear_nmi nmi_ctl_int(
 .i_clk					(clk_50m			),  // 输入：系统时钟
 .i_rst_n				(pon_reset_n		),  // 输入：全局复位（高电平有效，复位时中断控制模块初始化）
 .i_clr_nmi_flg			(w_bmc_nmi_ctl		),  // 输入：NMI 触发信号（来自 BMC I2C 寄存器 0x03A0 bit6，1=触发中断）
 .i_t64ms_tick			(t1s_tick			),  // 输入：时钟使能（1秒周期，控制中断信号的持续时间）
 .o_clr_nmi_done		(w_bmc_nmi_ctl_done	),  // 输出：NMI 触发完成标志（1=中断已发送，反馈给 BMC）
 .o_clr_nmi_done_rst	(w_bmc_nmi_ctl_rst	)  // 输出：NMI 中断信号（1=向 CPU 发送 NMI 中断）
);
// NMI 中断信号映射：向 CPU0/CPU1 发送 NMI 中断（低电平有效，1=中断解除，0=触发中断）
assign  w_p0_nmi_sync_flood_n = ~w_bmc_nmi_ctl_done; 
assign  w_p1_nmi_sync_flood_n = ~w_bmc_nmi_ctl_done; 
// 非屏蔽特性：NMI 是 CPU 不可屏蔽的中断，即使系统死机，也能触发 CPU 执行预设的故障处理程序（如保存内存日志、重启系统）；
// 应用场景：当 BMC 检测到严重硬件故障（如内存错误、CPU 过热）时，触发 NMI 中断，CPU 执行故障诊断流程，避免故障扩大；
// 信号映射： w_p0_nmi_sync_flood_n/w_p1_nmi_sync_flood_n 分别向 CPU0/CPU1 发送中断信号，支持双 CPU 平台的故障协同处理

// assign o_EPR_WP_N    	= eeprom_wp;			
// assign bmc_alarm_flag	= |bmc_wdt_rst_evt;

// 内存告警标志：当任意内存电源故障时置1（反馈给 BMC 或指示灯）
...
//-------------------------------------------------------------------------------------------------
// CPU0 PE0 --- switch board signal PVT_DATA   74lv165  系统电源稳定延迟：确保5V主电源与CPU休眠解除后，延迟1.5s/2s再执行后续动作（如扩展板状态采集）
//-------------------------------------------------------------------------------------------------
// 1. 2秒延迟：输入信号稳定后延迟2秒（2个1秒周期）
edge_delay #(
    .CNTR_NBITS(3),    // 3位计数器（0-7，满足2秒计数需求）
    .DELAY_MODE(1'b0)  // 上升沿延迟
) edge_delay_dc_pwr_on_2s (   
    .clk         (clk_50m),
    .reset       (~pon_reset_n),
    .cnt_size    (3'd2),            // 计数阈值=2（2*1s=2s）
    .cnt_step    (t1s_tick),        // 计数步长=1秒时钟使能
    .signal_in   (db_i_p0_slp_s3_n & w_all_power_pg), // 输入：触发条件（CPU0 S3休眠解除 AND 5V主电源就绪）
    .delay_output(w_pwr_on_dly2s)   // 输出：2秒延迟后的信号（用于ZT板状态采集复位）
);

// 2. 1.5秒延迟：输入信号稳定后延迟1.5秒（3个512ms周期，3*512ms=1536ms≈1.5s）
edge_delay #(.CNTR_NBITS(3), .DELAY_MODE(1'b0)) edge_delay_dc_pwr_on_1s5 (   
    .clk         (clk_50m),
    .reset       (~pon_reset_n),
    .cnt_size    (3'd3),            // 计数阈值=3（3*512ms≈1.5s）
    .cnt_step    (t512ms_tick),     // 计数步长=512ms时钟使能
    .signal_in   (db_i_p0_slp_s3_n & w_all_power_pg), // 输入：同2秒延迟的触发条件
    .delay_output(w_pwr_on_dly1s5)  // 输出：1.5秒延迟后的信号（预留，用于其他扩展板控制）
);
//触发条件严格：仅当 “CPU S3 休眠解除（db_i_p0_slp_s3_n=1，CPU 进入工作状态）” 且 “5V 主电源就绪（db_i_pgd_p5v=1，外设供电稳定）” 时，才启动延迟 —— 确保后续硬件状态采集在 “系统整体稳定” 后进行，避免采集到不稳定状态（如电压波动导致的虚假告警）；
//秒级延迟需求：ZT 板、Switch 板等扩展板的状态采集芯片（如 74LV165 移位寄存器）需 2 秒左右完成初始化，w_pwr_on_dly2s 用于延迟解除采集模块的复位，确保采集数据准确。
...
//pvt_gpi 是 通用输入状态采集模块，通过 “串行移位寄存器（如 74LV165）” 采集扩展板（Switch 板、ZT 板）的多路硬件状态（在位、电源、告警）
//将并行状态转换为串行数据，减少硬件布线（仅需 CLK/LD/SDI 三根线）。
pvt_gpi #(
  .TOTAL_BIT_COUNT(64),
  .DEFAULT_STATE(64'h0),
...
//-------------------------------------------------------------------------------------------------
// 时序逻辑：锁存ZT板的关键状态（板卡ID、版本、MCIO槽在位）
always@(posedge clk_50m or negedge pon_reset_n) begin //2024-9-6 add 
    if(~pon_reset_n) 
    begin
            r_board_id0_zt     <= w_board_id0_zt     ;// 锁存ZT板ID第0位（8位ID的最低位）
            ...
            r_pcb_version2_zt  <= w_pcb_version2_zt  ;// 锁存ZT板PCB版本第2位（3位版本号）
            r_pcb_version1_zt  <= w_pcb_version1_zt  ;
            r_pcb_version0_zt  <= w_pcb_version0_zt  ;
            r_pca_version2_zt  <= w_pca_version2_zt  ;// 锁存ZT板PCA版本第2位（3位版本号）
            r_pca_version1_zt  <= w_pca_version1_zt  ;
            r_pca_version0_zt  <= w_pca_version0_zt  ;
            r_mcio9_prsnt_n_zt <= w_mcio9_prsnt_n_zt ;// 锁存MCIO9槽在位状态（0=有卡，1=无卡）
	...
	else  begin  // 正常工作状态：根据1.5秒延迟信号决定是否更新状态
	    if(w_pwr_on_dly1s5) // w_pwr_on_dly1s5=1：ZT板已上电稳定（1.5秒延迟后），锁存状态不再更新
            begin
                r_board_id0_zt     <= r_board_id0_zt     ;// 保持已锁存的ID
            ...
            else begin// w_pwr_on_dly1s5=0：ZT板未稳定，持续更新状态（跟踪实时采集值）
...
// 延迟锁存：w_pwr_on_dly1s5 是 1.5 秒延迟信号（来自前文 edge_delay 模块），确保 ZT 板上电 1.5 秒稳定后再固定状态，避免初期波动导致的 “有卡识别为无卡”“版本号错误” 等问题；
// 复位初始化：复位时先锁存当前状态，避免寄存器初始值为不定态（x 态）导致的逻辑错误；
// 状态稳定性：稳定后不再更新状态，确保 BIOS/BMC 读取的是 “最终稳定状态”，而非实时波动的瞬时值。

//-------------------------------------------------------------------------------------------------
// ZT board or  switch board  judge
//-------------------------------------------------------------------------------------------------
// 组合锁存的ZT板ID：将8位锁存ID拼接为完整的板卡ID（r_board_id7_zt是最高位）
assign  w_zt_board_id ={r_board_id7_zt,r_board_id6_zt,r_board_id5_zt,r_board_id4_zt,
                        r_board_id3_zt,r_board_id2_zt,r_board_id1_zt,r_board_id0_zt}; 

// 组合Switch板ID（预留，用于后续Switch板识别，当前未实际采集）
...
// 时序逻辑：判断ZT板是否在位（低电平有效，0=在位，1=不在位）
always@(posedge clk_50m or negedge pon_reset_n) begin
    if(~pon_reset_n) begin
		r_zt_board_prsnt_n <= 1'b0; // 复位时默认“在位”（避免初始状态误判）
	end
	else if (w_zt_board_id == 8'h2d) begin  // 关键判断：若锁存的ZT板ID=0x2d（预设ZT板ID），则判定为“在位”
	    r_zt_board_prsnt_n <= 1'b0;  // 0=ZT板在位
	end
	else begin  // 板卡ID≠0x2d（可能是Switch板或无板），判定为“不在位”
	    r_zt_board_prsnt_n <= 1'b1;  // 1=ZT板不在位
	end
end

// 输出ZT板在位状态（供后续信号映射使用）
assign w_zt_board_prsnt_n = r_zt_board_prsnt_n;
// ID 预设：硬件设计时为 ZT 板分配唯一 ID（8'h2d ），通过 “ID 匹配” 判断板卡类型，比 “单纯检测在位引脚” 更精准（避免无板时的噪声误触发）；
// 低电平有效：遵循服务器硬件设计惯例（_n 表示低有效），w_zt_board_prsnt_n=0 明确表示 ZT 板已安装且正常，便于后续逻辑统一处理；
// 预留扩展：w_sw_board_id 预留 Switch 板 ID 组合，为后续 “ZT 板 / Switch 板自动区分” 提供扩展空间


//2024-4-30 SW CHG VB REMAPPED
// 1. 电源告警信号映射（如TPM431安全芯片告警、INA3221电源监控告警）
...
// 2. 1-8号扩展槽在位状态映射（Switch板的槽位状态复用ZT板对应槽位）
...
// 3. MCIO槽在位状态映射（需同时判断ZT板和Switch板ID，适配不同板卡版本）
...
//     .cnt_size    (3'd2),
//     .cnt_step    (t1s_tick),
//     .signal_in   (db_i_p0_slp_s3_n & w_all_power_pg),
...
assign  w_PWRGD_P12V =   w_PWRGD_P12V_PS3_PS4 ;
...
assign  w_pal_db2000_1_pwrgd_r    = 1'b1 ;
assign  w_pal_db2000_2_pwrgd_r    = 1'b1 ;
assign  w_clk_db2000_1_1_oe_n     = 1'b0 ;
assign  w_clk_db2000_1_2_oe_n     = 1'b0 ;
...
assign  w_usb2_lcd_oc_n                         = i_P0_USB_OC2_N_R ;
//-----------------------------------------------------------------------------------------------//
//Output SIGNAL
//-----------------------------------------------------------------------------------------------//
...
assign  o_CLK_DB2000_1_1_OE_N                             =   1'b0;//8
assign  o_CLK_DB2000_1_2_OE_N                             =   1'b0;//9
...
assign  o_PAL_SATA1_CONTROLLER0_R                       =   1'bz;//52
assign  o_P0_PWR_BTN_R_N                                       =   w_pal_pwrbtn_n_r;
assign  o_P1_PWR_BTN_N                                         =   w_pal_pwrbtn_n_r;
assign  o_P0_SYS_RESET_R_N                                   =   w_p0_sys_reset_r_n;
assign  o_P1_SYS_RESET_N                                     =   w_p0_sys_reset_r_n;
...
// assign  o_P0_UART_RXD_1			                     = w_st_steady_pwrok ? i_UART_SYS_RX_R: 1'b0;			
assign  o_UART_SYS_TX_R			                     = i_P0_UART_TXD_1;
...
assign  o_P0_VDD_3V3_STBY_EN_R                        =   w_grp_b_p0_33_s5_en;
assign  o_P1_VDD_3V3_STBY_EN_R                        =   w_grp_b_p1_33_s5_en;
assign  o_PAL_P0_VDD_18_STBY_EN_R                     =   w_grp_b_p0_18_s5_en;
assign  o_PAL_P1_VDD_18_STBY_EN_R                     =   w_grp_b_p1_18_s5_en;
...
assign  o_PAL_P12V_CPU1_ON                             =   w_pal_p12v_cpu1_on  ;
assign  o_PAL_P12V_CPU0_ON                             =   w_pal_p12v_cpu0_on  ;
...
assign  o_P0_VPP_9545_3_RST_N_R                         =   w_p0_vpp_9545_3_rst_n;
assign  o_P0_VPP_9545_4_RST_N_R                         =   w_p0_vpp_9545_4_rst_n;
assign  o_P0_VPP_9545_5_RST_N_R                         =   w_p0_vpp_9545_5_rst_n;
assign  o_P0_VPP_9545_6_RST_N_R                         =   w_p0_vpp_9545_6_rst_n;
assign  o_PAL_P0_I2C5_9548_RST_R                       =   w_bmc_i2c5_9548_rst_n;
assign  o_BMC_I2C4_9548_1_RST_N_R                     =   w_bmc_i2c4_9548_1_rst_n;
assign  o_BMC_I2C4_9548_2_RST_N_R                     =   w_bmc_i2c4_9548_2_rst_n;
assign  o_BMC_I2C4_9548_3_RST_N_R                     =   w_bmc_i2c4_9548_3_rst_n;
assign  o_BMC_I2C4_9548_4_RST_N_R                     =   w_bmc_i2c4_9548_4_rst_n;
...
assign  o_PAL_TPM_EN                             =   1'bz  ;
assign  o_EEPROM_WP_N_R                                         =   w_eeprom_wp ? 1'b0 : 1'b1;
assign  o_PAL_PVCC_HPMOS_EN_R                          =   w_grp_b_pal_pvcc_hpmos_en  ;
assign  o_SPI1_BMC_WP_N_1V8                             =   1'bz  ;
assign  o_PAL_RST_RTC                             =   w_pal_rst_rtc  ;
//--------------------------------------------------------------------------------------------------------------------------------------------------
//I2C RAM  Start
//--------------------------------------------------------------------------------------------------------------------------------------------------
/************************************************************************************************************************************************************************/
...
.i_1ms_clk		(t1ms_tick		),	          
.i_rst_i2c_n	(1'b1			),		
.i_scl			(i_I2C2_PAL_SCL	), 
.io_sda			(io_I2C2_PAL_SDA),
...
// //PSU--0x000D
// .i_PS1_PRSNT                              (db_i_ps1_prsnt)                  , //addr 0x000D bit7  PS1 电源模块存在检测
...
// .i_PS1_ACFAIL                            (db_i_ps1_acfail_n)            , //addr 0x000D bit3  PS1 交流输入故障（低电平有效)
// .i_PS2_ACFAIL                            (db_i_ps2_acfail_n)            , //addr 0x000D bit2
// .i_PS1_DCOK                                (db_i_ps1_dcok_n)                , //addr 0x000D bit1  PS1 直流输出正常（低电平有效)
// .i_PS2_DCOK                                (db_i_ps2_dcok_n)                , //addr 0x000D bit0
// //PSU--0x000E
// .i_PS1_ALERT                              (db_i_ps1_smb_alert)                , //addr 0x000E bit7  PS1 告警信号
// .i_PS2_ALERT                              (db_i_ps2_smb_alert)                , //addr 0x000E bit6
// .i_PS1_P12V_ON                          (w_ps1_p12v_on_r)                     , //addr 0x000E bit5 PS1 的 12V 电源使能状态
// .i_PS2_P12V_ON                          (w_ps2_p12v_on_r)                     , //addr 0x000E bit4
// .i_PS_OFF                                    (w_pal_ps_off_r)                 , //addr 0x000E bit3  全局电源关闭控制
// .i_DUAL_EN                                  (w_pal_dual_en_r)                 , //addr 0x000E bit2  双 PSU 冗余使能
...
.i_p1_pwr_good_r			        (w_cpu1_pwr_good			),	//addr 0x0024 bit5
//CPU0 PWR EN --0x0025
...
.i_pal_pvcc_hpmos_en        (w_grp_b_pal_pvcc_hpmos_en ), //addr 0x0025 bit0
//CPU PRSNT --0x0030
...
.i_FM_CPU_SMERR_LVC3_N_R                           (db_i_fm_cpu_smerr_lvc3_n_r             ),      //addr 0x0032 bit5
//CPU THERM --0x0033
...
//0x0065
.i_p1_vr_i2c_alert_n			(~db_i_p1_vr_i2c_alert_n),	//addr 0x0065 bit7
.i_p0_vr_i2c_alert_n			(~db_i_p0_vr_i2c_alert_n),	//addr 0x0065 bit6		
//0x0066
...
//i2c_mux_rst -- 0x0077
.o_p0_vpp_9545_4_rst_n			(w_p0_vpp_9545_4_rst_n	),	//addr 0x0077 bit7
.o_p0_vpp_9545_5_rst_n			(w_p0_vpp_9545_5_rst_n	),	//addr 0x0077 bit6
.o_p0_vpp_9545_6_rst_n			(w_p0_vpp_9545_6_rst_n	),	//addr 0x0077 bit5
//i2c_mux_rst -- 0x0078
.o_bmc_i2c5_9548_rst_n			(w_bmc_i2c5_9548_rst_n	),	//addr 0x0078 bit7
.o_bmc_i2c4_9548_1_rst_n		(w_bmc_i2c4_9548_1_rst_n),	//addr 0x0078 bit6
.o_bmc_i2c4_9548_2_rst_n		(w_bmc_i2c4_9548_2_rst_n),	//addr 0x0078 bit5
.o_bmc_i2c4_9548_3_rst_n		(w_bmc_i2c4_9548_3_rst_n),	//addr 0x0078 bit4
.o_bmc_i2c4_9548_4_rst_n		(w_bmc_i2c4_9548_4_rst_n),	//addr 0x0078 bit3
.o_p0_vpp_9545_1_rst_n			(w_p0_vpp_9545_1_rst_n	),	//addr 0x0078 bit2
.o_p0_vpp_9545_2_rst_n			(w_p0_vpp_9545_2_rst_n	),	//addr 0x0078 bit1
.o_p0_vpp_9545_3_rst_n			(w_p0_vpp_9545_3_rst_n	),	//addr 0x0078 bit1
...
//0x0091
.i_p12v_stby_fault_det			(1'b0				),	//addr 0x0091 bit7
.i_p5v_stby_fault_det			(w_p5v_stby_fault_det				),	//addr 0x0091 bit6
.i_grp_b_pal_pvcc_hpmos_fault_det      (w_grp_b_pal_pvcc_hpmos_fault_det			),	//addr 0x0091 bit4
...
.o_pal_rst_rtc                           (w_pal_rst_rtc        ),	//addr 0x009E bit7
...
.i_cpu_sys_reset_r_n				(db_i_fm_rstbtn_out_n_r	),	//addr 0x0103 bit1
...
.o_espi_ram_1054                        (w_espi_ram_1054)                   , //addr 0x1054 //default 0xff
.i_espi_ram_1055                        (w_espi_ram_1055)                   , //addr 0x1055 //default 0xff
.i_espi_ram_1056                        (w_espi_ram_1056)                   , //addr 0x1056 //default 0xff
.i_espi_ram_1057                        (w_espi_ram_1057)                   , //addr 0x1057 //default 0xff
.i_espi_ram_1058                        (w_espi_ram_1058)                     //addr 0x1058 //default 0xff
