...
//base signal  Clocks and resets
 ...
 //from Power Controller PG signal   
  ...
  input     wire    grp_b_pal_pvcc_hpmos_pg   ,             
...
//Fault Detect Signal  
  ...
  output    wire    grp_b_pal_pvcc_hpmos_fault_det	,
  ...
  output    wire    grp_b_pal_pvcc_hpmos_en	, 
  output    wire    power_supply_on		,
  output    wire    pal_p12v_cpu0_on,
  output    wire    pal_p12v_cpu1_on,
...
reg reg_grp_b_pal_pvcc_hpmos_en	; // CPU B组PAL_PVCC_HPMOS使能目标状态 o_PAL_PVCC_HPMOS_EN_R
...
reg reg_pal_p12v_cpu0_en     ; // CPU0 12V高压MOS管使能目标状态，1=开启，0=关闭
reg reg_pal_p12v_cpu1_en     ; // CPU1 12V高压MOS管使能目标状态，1=开启，0=关闭
reg reg_p12_en_p0_dimm    ; // CPU0 DIMM 12V使能目标状态，1=开启，0=关闭
reg reg_p12_en_p1_dimm    ; // CPU1 DIMM 12V使能目标状态，1=开启，0=关闭
...
assign grp_b_p0_33_s5_en	        =	reg_grp_b_33_s5_en		&	~i_cpu_prsnt_n[0] & (~grp_b_p0_33_s5_fault_det | keep_alive_on_fault )	;//~i_cpu_prsnt_n[0] & (~grp_b_p0_33_s5_fault_det | keep_alive_on_fault )	;
assign grp_b_p1_33_s5_en	        =	reg_grp_b_33_s5_en		&	~i_cpu_prsnt_n[1] & ~i_cpu_prsnt_n[0] & (~grp_b_p1_33_s5_fault_det | keep_alive_on_fault )	;
assign grp_b_p0_18_s5_en	        =	reg_grp_b_18_s5_en		&	~i_cpu_prsnt_n[0] & (~grp_b_p0_18_s5_fault_det | keep_alive_on_fault )	;//~i_cpu_prsnt_n[0] & (~grp_b_p0_18_s5_fault_det | keep_alive_on_fault )	;
assign grp_b_p1_18_s5_en	        =	reg_grp_b_18_s5_en		&	~i_cpu_prsnt_n[1] & ~i_cpu_prsnt_n[0] & (~grp_b_p1_18_s5_fault_det | keep_alive_on_fault )	;
assign grp_b_pal_pvcc_hpmos_en	  =	reg_grp_b_pal_pvcc_hpmos_en	& 	~i_cpu_prsnt_n[0]	& (~grp_b_pal_pvcc_hpmos_fault_det | keep_alive_on_fault )	;
// 12V主电源使能：目标状态 + 故障保持（特殊逻辑：故障时强制保持，避免主电源频繁开关）
assign power_supply_on		=	reg_power_supply_on	| keep_alive_on_fault 	;


assign pal_p12v_cpu0_on       =   reg_pal_p12v_cpu0_en     &   ~i_cpu_prsnt_n[0]   ;
assign pal_p12v_cpu1_on       =   reg_pal_p12v_cpu1_en     &   ~i_cpu_prsnt_n[1] & ~i_cpu_prsnt_n[0]  ;
assign p12_en_p0_dimm_1		=	reg_p12_en_p0_dimm		&	~i_cpu_prsnt_n[0]  	;
assign p12_en_p1_dimm_1		=	reg_p12_en_p1_dimm		&	~i_cpu_prsnt_n[1] & ~i_cpu_prsnt_n[0]  	;
...
assign grp_c_p1_vdd11_en	        =	reg_grp_c_en			&	~i_cpu_prsnt_n[1] &  ~i_cpu_prsnt_n[0] & (~grp_c_p1_fault_det			 | keep_alive_on_fault )	;
...
assign grp_d_p1_vddio_en	        =	reg_grp_d_vddio_en		&	~i_cpu_prsnt_n[1] & ~i_cpu_prsnt_n[0] & (~grp_d_vddio_p1_fault_det 	 | keep_alive_on_fault )	;
assign grp_d_p0_soc_en		=	reg_grp_d_soc_en		&	~i_cpu_prsnt_n[0] & (~grp_d_soc_p0_fault_det		 | keep_alive_on_fault )	;
assign grp_d_p1_soc_en		=	reg_grp_d_soc_en		&	~i_cpu_prsnt_n[1] & ~i_cpu_prsnt_n[0] & (~grp_d_soc_p1_fault_det		 | keep_alive_on_fault )	;
assign grp_d_p0_vddcore0_en	=	reg_grp_d_vddcore0_en	&	~i_cpu_prsnt_n[0] & (~grp_d_p0_vddcore0_fault_det | keep_alive_on_fault )	;
assign grp_d_p1_vddcore0_en	=	reg_grp_d_vddcore0_en	&	~i_cpu_prsnt_n[1] & ~i_cpu_prsnt_n[0] & (~grp_d_p1_vddcore0_fault_det | keep_alive_on_fault )	;
assign grp_d_p0_vddcore1_en	=	reg_grp_d_vddcore1_en	&	~i_cpu_prsnt_n[0] & (~grp_d_p0_vddcore1_fault_det | keep_alive_on_fault )	;
assign grp_d_p1_vddcore1_en	=	reg_grp_d_vddcore1_en	&	~i_cpu_prsnt_n[1] & ~i_cpu_prsnt_n[0] & (~grp_d_p1_vddcore1_fault_det | keep_alive_on_fault )	;
...
always @(posedge clk or posedge reset) begin
  if (reset) begin
              ...
              reg_grp_b_pal_pvcc_hpmos_en  <= 1'b0;
              reg_power_supply_on		<= 1'b0;
              reg_pal_p12v_cpu0_en     <= 1'b0;
              reg_pal_p12v_cpu1_en     <= 1'b0;
              reg_p12_en_p0_dimm       <= 1'b0;
              reg_p12_en_p1_dimm       <= 1'b0;
              reg_p5v_en				<= 1'b0;
              reg_grp_c_en				<= 1'b0;
    ...
    case (power_seq_sm)
      ...
      SM_EN_GRP_B_18_S5 : begin
            ...
            // reg_grp_b_pal_pvcc_hpmos_en  <= 1'b1;   
      end
	...
	//no action execution
      ...
      SM_EN_MAIN_EFUSE : begin
            ...
            reg_pal_p12v_cpu0_en     <= 1'b1;
            reg_pal_p12v_cpu1_en     <= 1'b1;
            reg_p12_en_p0_dimm       <= 1'b1;
            reg_p12_en_p1_dimm       <= 1'b1;
...
//------------------------------------------------------------------------------
// PAL_PVCC_HPMOS_S5 & P1_VDD_33_S5 Fault detect 
//------------------------------------------------------------------------------
wire grp_b_pal_pvcc_hpmos_en_check;

edge_delay #(.CNTR_NBITS(2)) grp_b_pal_pvcc_hpmos_en_check_inst (
  .clk           (clk),
  .reset         (reset),
  .cnt_size      (2'b10),
  .cnt_step      (t64ms),
  .signal_in     (grp_b_pal_pvcc_hpmos_en),
  .delay_output  (grp_b_pal_pvcc_hpmos_en_check)
);

fault_detectB_chklive #(.NUMBER_OF_VRM(1)) grp_b_pal_pvcc_hpmos_fault_det_inst (
  .clk              (clk),								//in
  .reset            (reset),							//in
  .vrm_enable       (grp_b_pal_pvcc_hpmos_en && grp_b_pal_pvcc_hpmos_en_check),	//in
  .vrm_pgood        ( grp_b_pal_pvcc_hpmos_pg),						//in
  .vrm_chklive_en   (grp_b_pal_pvcc_hpmos_en_check),						//in
  .vrm_chklive_dis  (~grp_b_pal_pvcc_hpmos_en_check),					//in
  .critical_fail    (st_critical_fail),					//in
  .fault_clear      (fault_clear),						//in
  .lock             (any_pwr_fault_det),				//in
  .any_vrm_fault    (),									//out
  .vrm_fault        (grp_b_pal_pvcc_hpmos_fault_det)			//out
);
...
assign fault_vec[8]   = grp_b_pal_pvcc_hpmos_fault_det;// 位8：PAL CPU B组高性能MOSFET电源故障（如1.8V/2.5V HPMOS供电）
...
assign fault_vec[39:25]  = 'd0;                          // 位25：预留（用于扩展新电源通道）
