...
// Physical power button and south bridge status/control
  ...
  input   wire  pwr_btn_long,                     // 电源按钮长按检测信号，指示用户长时间按下电源按钮
  // Misc.
...
begin
        ...
        begin
                r_pwr_btn_dly1  <= 1'b0;  // 1级延时（当前周期的滤波后信号）
                r_pwr_btn_dly2  <= 1'b0;  // 2级延时（上一周期的滤波后信号）
        end
        else  // 正常工作：每时钟周期更新延时寄存器（移位操作）
    begin
        r_pwr_btn_dly1 <= p0_pwrbtn_n    ; // 1级延时 = 当前滤波后信号
        r_pwr_btn_dly2 <= r_pwr_btn_dly1 ; // 2级延时 = 上一周期的1级延时信号
    end
end

// 上升沿检测（按钮按下）：上一周期为高（r_pwr_btn_dly2=1），当前周期为低（r_pwr_btn_dly1=0）
assign w_pwr_btn_pos = (r_pwr_btn_dly1) && (!r_pwr_btn_dly2) ;
// // 下降沿检测（按钮松开）：上一周期为低（r_pwr_btn_dly2=0），当前周期为高（r_pwr_btn_dly1=1）
...
//         else begin
//                 // 按键松开时（下降沿），计数清零
//                 if(w_pwr_btn_pos)                                                   r_cnt_pwr_btn        <= 8'h00;
//                 // 计数达到阈值（PWRBTN_LONG×10+1），停止计数
//                 else if ( r_cnt_pwr_btn == (PWRBTN_LONG*10 +1))  r_cnt_pwr_btn        <= r_cnt_pwr_btn ;
//                 // 按键按下且100ms脉冲到来，计数+1
//                 else if ( p0_pwrbtn_n & r_clk_100ms   )    r_cnt_pwr_btn        <= r_cnt_pwr_btn + 1'b1 ;
...
wire st_ps_on;      // 状态标志：是否处于SM_PS_ON（PSU使能）
...
always @(posedge clk or posedge reset) begin 
  if (reset)
    power_seq_sm <= power_seq_sm;// 复位时保持初始值
  else 
     begin
...
always @(*) begin
  ...
  case (state)
    ...
    SM_STEADY_PWROK : begin         //6'h20
          ...
          if ((rt_normal_pwr_down || r_Pwrbtn_long) ) begin //rt_normal_pwr_down：正常下电请求（如南桥 S4 状态触发，优先级次高）
            //  if (( r_Pwrbtn_long) ) begin
