...
begin
  if (reset)
    ...
    pch_pwrbtn <= ~gmt_wakeup_n |
                  (~force_off & ~sys_sw_in_n & pwr_btn_allow);
end
...
begin
  if (reset)
    ...
    shutdown_events <= {shutdown_events[0], (cpu_thermtrip |
                                             pch_thermtrip |
                                             temp_deadly  |
                                             gmt_shutdown |
                                             force_off)};
