...
input  wire        i_pal_pvcc_hpmos_en         , //addr 0x0025 bit0
...
input   wire  i_grp_b_pal_pvcc_hpmos_fault_det, //addr 0x0091 bit4
...
assign w_ram_0091 = {
					...
					1'b0                            ,
					i_grp_b_pal_pvcc_hpmos_fault_det ,
...
begin
	...
	begin
	case(w_i2c_command)// 解析BMC发送的16位寄存器地址
...
//i2c slave
///////////////////////////////////////////////////////////////////////
//模块通过例化 i2c_slave_bmc 实现 I2C 从机功能，核心是 “解析 BMC 命令→读写寄存器→返回数据” 的交互流程
i2c_slave_bmc  #(
.DLY_LEN                 (DLY_LEN)      //3   //24.18MHz,330ns
    ...
    .i_i2c_address              (7'h20                      ),
    .o_i2c_start                (w_i2c_start          ),// 输出：检测到I2C起始信号
    .o_WR                       (w_WR                        ),// 输出：I2C读写标志（0=写，1=读）
    .o_data_vld_pos             (w_data_vld_pos    ),// 输出：主机写入数据有效标志（高电平表示数据可用）
    .o_i2c_command              (w_i2c_command      ),// 输出：BMC发送的命令（16位寄存器地址）
    .i_i2c_data_in              (r_i2c_data_in      ),// 输入：从机返回给主机的数据（寄存器值）
    .o_i2c_data_out             (w_i2c_data_out    )// 输出：主机写入从机的数据
); 
