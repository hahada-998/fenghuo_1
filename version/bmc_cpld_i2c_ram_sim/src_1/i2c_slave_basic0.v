...
module i2c_slave_basic0 #(
    ...
    input  wire [7:0]       i_data_in,            // 并行数据输入，用于读/写操作
    input  wire             i_addr_hit,           // 地址匹配信号，高电平表示匹配成功
    output reg [6:0]        o_I2C_ADDR_OUT,       // 匹配的 I2C 从设备地址输出
    output reg [7:0]        o_data_out,           // 并行读/写寄存器“地址数据”输出，用于读/写操作
...
begin
    if (w_rst)
        ...
        case (r_I2C_state)
            ...
            5'h12: if (w_scl_neg) r_I2C_state <= (r_I2C_RW && w_r1_sda_dly ? 5'h12 : 5'h0A); // 读操作且主机SDA发送1, 则停止读，否则返回数据接收状态
...
begin
    if (w_rst)
        ...
        case (r_I2C_state)
            5'h09: r_sda_en <= i_addr_hit; // 地址匹配时使能 ACK 输出
            5'h0A: r_sda_en <= r_I2C_RW  ; // 读/写从设备地址操作完成使能 ACK 输出
            5'h12: r_sda_en <= ~r_I2C_RW ; // 读/写从设备寄存器地址操作完成时, 写发送 ACK 输出, 读操作禁用 SDA 输出
...
// 控制 SDA 输出地址 r_I2c_address
always @(posedge i_clk or posedge w_rst) begin
    if (w_rst) begin
        r_I2c_address <= 7'b0; // 复位时清零
    end else begin
        case (r_I2C_state)
            5'h01: if (w_scl_neg) r_I2c_address[6] <= w_r1_sda_dly;
            5'h02: if (w_scl_neg) r_I2c_address[5] <= w_r1_sda_dly;
            5'h03: if (w_scl_neg) r_I2c_address[4] <= w_r1_sda_dly;
            5'h04: if (w_scl_neg) r_I2c_address[3] <= w_r1_sda_dly;
            5'h05: if (w_scl_neg) r_I2c_address[2] <= w_r1_sda_dly;
            5'h06: if (w_scl_neg) r_I2c_address[1] <= w_r1_sda_dly;
            5'h07: if (w_scl_neg) r_I2c_address[0] <= w_r1_sda_dly;
            default: r_I2c_address <= r_I2c_address; // 保持原值
        endcase
    end
end
...
begin
    ...
    else if (w_scl_neg) begin
        case (r_I2C_state)
            5'h0A: o_data_out[7] <= w_r1_sda_dly; // 接收数据的最高位
            5'h0B: o_data_out[6] <= w_r1_sda_dly;
            5'h0C: o_data_out[5] <= w_r1_sda_dly;
            5'h0D: o_data_out[4] <= w_r1_sda_dly;
            5'h0E: o_data_out[3] <= w_r1_sda_dly;
            5'h0F: o_data_out[2] <= w_r1_sda_dly;
            5'h10: o_data_out[1] <= w_r1_sda_dly;
            5'h11: o_data_out[0] <= w_r1_sda_dly; // 接收数据的最低位
            default: o_data_out <= o_data_out; // 保持原值
        endcase
    end
end
