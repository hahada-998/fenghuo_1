...
`include "YRS36M2C4S_M_define.vh"
...
//parameter [2*NUM_CPU-1:0] HPMOS_OWNER = 4'b00_00,
  ...
  input                    t512us,                            // 10ns pulse every 5ms
 ...
 //from pwrseq_master 
  ...
  input 				   dc_on_wait_complete,				  //in FROM MASTER
  input 				   rt_critical_fail_store,			  //in FROM MASTER
  input					   fault_clear,						  //in FROM MASTER 
  
  
 //to pwrseq_master   
  output reg				pgd_so_far				    ,  
  output 					s5dev_pwren_request		    ,       	
  output 					s5dev_pwrdis_request	    ,           
 ...
 //from Power Controller PG signal   
  input 					p5v_stby_pg  		        ,
  input 					grp_b_p0_33_s5_pg	        ,
  input 					grp_b_p1_33_s5_pg	        ,
  input 					grp_b_p0_18_s5_pg	        ,
  input 					grp_b_p1_18_s5_pg	        ,                
  input  					p3v3_stby_pg                ,//20220109 c00268;idms202201040006
  input  					p12v_stby_pg                ,//20220109 c00268;idms202201040006
  
  
  input 					p12v_efuse_pg		        ,
  input 					p12v_ssd_efuse_pg	        , 
  input 					p12v_p0_dimm_pg		        , 
  input 					p12v_p1_dimm_pg		        ,  
  input 					p3v3_pg				        ,
  input 					grp_c_p0_pg			        ,
  input 					grp_c_p1_pg			        ,
  input 					grp_d_vddio_p0_pg	        ,  
  input 					grp_d_vddio_p1_pg	        ,
  input 					grp_d_soc_p0_pg		        ,
  input 					grp_d_soc_p1_pg		        ,  
  input 					grp_d_p0_vddcore0_pg	    ,
  input 					grp_d_p1_vddcore0_pg	    ,  
  input 					grp_d_p0_vddcore1_pg	    ,
  input 					grp_d_p1_vddcore1_pg	    ,
  
  input						i_pwrgd_ocp0_nic_pwrgd	    ,
  
  input                [2:0]pcb_id                      ,//20220725 d00412
  
//Fault Detect Signal  
  output reg         [5:0]  pwrseq_sm_fault_det			,    // SM state where fault occurred
  output					p5v_stby_fault_det			,
  output					grp_c_p0_fault_det			,
  output					grp_c_p1_fault_det			,  
  output					grp_d_vddio_p0_fault_det	, 
  output					grp_d_vddio_p1_fault_det	,   
  output					grp_d_soc_p0_fault_det		,  
  output					grp_d_soc_p1_fault_det		,  
  output					grp_d_p0_vddcore0_fault_det	,
  output					grp_d_p1_vddcore0_fault_det	,  
  output					grp_d_p0_vddcore1_fault_det	,
  output					grp_d_p1_vddcore1_fault_det	,  
  
  
  output					grp_b_p0_33_s5_fault_det	,
  output					grp_b_p1_33_s5_fault_det	,
  output					grp_b_p0_18_s5_fault_det	,
  output					grp_b_p1_18_s5_fault_det	,
  
  output                    p3v3_stby_fault_det         ,//20220109 c00268;idms:202201040006
  output                    p12v_stby_fault_det         ,//20220109 c00268;idms:202201040006 
  
  output					p3v3_fault_det				,
  output					p12v_efuse_fault_det		,
  output					p12v_ssd_efuse_fault_det	,
  output					p12v_p0_dimm_fault_det		,
  output					p12v_p1_dimm_fault_det		, 


  
  //to Power Controller Enable Pin
  output				   p5v_stby_en      	,
  output				   p5v_stby_usb_en		,
  output				   grp_b_p0_33_s5_en	,
  output				   grp_b_p1_33_s5_en	,
  output				   grp_b_p0_18_s5_en	,
  output				   grp_b_p1_18_s5_en	,
  output				   power_supply_on		,
  output				   p12_en_p0_dimm_1		,//20220724 d00412
  output				   p12_en_p1_dimm_1		,//20220724 d00412
  output                   p12_en_p0_dimm_2     ,//20220724 d00412
  output                   p12_en_p1_dimm_2     ,//20220724 d00412
  output				   p3v3_en				,
  output				   grp_c_p0_vdd11_en	,
  output				   grp_c_p1_vdd11_en	,
  output				   grp_d_p0_vddio_en	,
  output				   grp_d_p1_vddio_en	,
  output				   grp_d_p0_soc_en		,
  output				   grp_d_p1_soc_en		,
  output				   grp_d_p0_vddcore0_en	,
  output				   grp_d_p1_vddcore0_en	,
  output 				   grp_d_p0_vddcore1_en	,
  output 				   grp_d_p1_vddcore1_en	,
  output				   ocp_aux_en			,
  output				   ocp_main_en			,
  ...
  //from CPU
  input  	 [NUM_CPU-1:0] i_cpu_pwrok				, 
  input  	 [NUM_CPU-1:0] i_cpu_prsnt_n			,	
  
  //to CPU  
  output    			   o_p0_pwr_good			, //for AMD PWR_GOOD
  output  	 [NUM_CPU-1:0] o_cpu_pwrok				,
  output  	               o_rsmrst_n				,
...
reg reg_rsmrst_l			;
reg reg_p5v_stby_en			;
reg reg_p5v_stby_usb_en     ; //20220224 d50092
...
reg reg_p12_en_dimm         ; //20220713 d00412


assign p5v_stby_en			=	reg_p5v_stby_en			&	( ~p5v_stby_fault_det 									| keep_alive_on_fault )	;
assign p5v_stby_usb_en  	=	reg_p5v_stby_usb_en		;//&((~p5v_stby_usb0_fault_det & ~p5v_stby_usb1_fault_det) 	| keep_alive_on_fault )	;  //20220224 d50092
...
assign p12_en_p0_dimm_1		=	reg_p12_en_dimm		    &	~i_cpu_prsnt_n[0] & (~p12v_p0_dimm_fault_det 			| keep_alive_on_fault )	; //20220713 d00412	
assign p12_en_p1_dimm_1		=	reg_p12_en_dimm		    &	~i_cpu_prsnt_n[1] & (~p12v_p1_dimm_fault_det 			| keep_alive_on_fault )	; //20220713 d00412
assign p12_en_p0_dimm_2		=	reg_power_supply_on		&	~i_cpu_prsnt_n[0] & (~p12v_p0_dimm_fault_det 			| keep_alive_on_fault )	; //20220724 d00412
assign p12_en_p1_dimm_2		=	reg_power_supply_on		&	~i_cpu_prsnt_n[1] & (~p12v_p1_dimm_fault_det 			| keep_alive_on_fault )	; //20220724 d00412

assign p3v3_en				=	reg_p3v3_en				&						(~p3v3_fault_det					| keep_alive_on_fault )	;
...
always @(posedge clk or posedge reset) begin
  if (reset) begin
		  ...
		  reg_p5v_stby_usb_en		<= 1'b0;   //20220224 d50092
		  ...
		  reg_p12_en_dimm           <= 1'b0;  //20220713 d00412
  end
    ...
    case (power_seq_sm)
	  ...
	  SM_OFF_STANDBY: begin
        //no action execution
		//reg_usb_ponrst_r_n		<= 1'b1; //20220812 d00412
		reg_tpcm_reset_n		<= 1'b1;
		reg_p5v_stby_en         <= 1'b1;//c00268 for hitless
		reg_p5v_stby_usb_en     <= 1'b1;//c00268 for hitless
        reg_grp_b_33_s5_en		<= 1'b1;//c00268 for hitless		
        reg_grp_b_18_s5_en		<= 1'b1;//c00268 for hitless
        reg_rsmrst_l            <= 1'b1;//c00268 for hitless
		reg_s5dev_pwren_request <= 1'b0;//c00268 for hitless
		reg_ocp_aux_en			<= 1'b1;//20220505 d00412 rdc:3735424 
		reg_p12_en_dimm         <= 1'b1;  //20220713 d00412
      end
	  ...
	  //no action execution
	  //c00268 for hitless
	    reg_power_supply_on    	   <= 1'b1;//c00268 for hitless	
	    reg_ocp_main_en		       <= 1'b1;	  
	    reg_p3v3_en                <= 1'b1;
		reg_grp_c_en			   <= 1'b1;
		reg_grp_d_vddio_en		   <= 1'b1;
		...
		reg_s5dev_pwren_request    <= 1'b0;	
		reg_usb_ponrst_r_n		   <= 1'b1;
		reg_tpcm_reset_n		   <= 1'b1;//c00268 for hitless	
        reg_ocp_aux_en			   <= 1'b1;//20220528 d00412 rdc:3735424
		reg_p12_en_dimm            <= 1'b1;  //20220713 d00412
	  end
...
edge_delay #(.CNTR_NBITS(2)) p3v3_en_check_inst (
  .clk           (clk),
  .reset         (reset),
  .cnt_size      (2'b10),
  .cnt_step      (t64ms),
  .signal_in     (p3v3_en),
  .delay_output  (p3v3_en_check)
);

fault_detectB_chklive #(.NUMBER_OF_VRM(1)) p3v3_fault_detect_inst (
  .clk              (clk),								//in
  .reset            (reset),							//in
  .vrm_enable       (p3v3_en && p3v3_en_check),			//in
  .vrm_pgood        (p3v3_pg),							//in
  .vrm_chklive_en   (p3v3_en_check),					//in
  .vrm_chklive_dis  (~p3v3_en_check),					//in
  .critical_fail    (st_critical_fail),					//in
  .fault_clear      (fault_clear),						//in
  .lock             (any_pwr_fault_det),				//in
  .any_vrm_fault    (),									//out
...
//------------------------------------------------------------------------------
// Stby_Power  Fault detect 20220108 c00268;idms:202201040006
//------------------------------------------------------------------------------
//p3v3_stby
wire   p3v3_stby_en;
...
assign fault_vec[1]   = 1'b0;//p5v_stby_usb0_fault_det   ; //20220223 d50092 rdc:3725674
assign fault_vec[2]   = 1'b0;//p5v_stby_usb1_fault_det   ; //20220223 d50092 rdc:3725674
assign fault_vec[3]   = 1'b0;
assign fault_vec[4]   = p3v3_fault_det             ;
assign fault_vec[5]   = p12v_efuse_fault_det       ;  
assign fault_vec[6]   = p12v_ssd_efuse_fault_det   ;
assign fault_vec[7]   = p12v_p0_dimm_fault_det     ;
assign fault_vec[8]   = p12v_p1_dimm_fault_det     ;
...
assign fault_vec[23]  = p3v3_stby_fault_det        ;//20220109 c00268;idms:202201040006
assign fault_vec[24]  = p12v_stby_fault_det        ;//20220109 c00268;idms:202201040006
