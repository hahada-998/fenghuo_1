...
always @(*) begin
  ...
  case (state)
    ...
    SM_PS_ON : begin                 //09  PSU 使能
          ...
          // else 
          if (psu_watchdog_timeout && pgd_so_far) begin
    ...
    SM_EN_MAIN_EFUSE : begin         //11 主 eFuse 使能
          ...
          // else 
          if (efuse_watchdog_timeout && pgd_so_far) begin
    ...
    SM_WAIT_POWEROK : begin         //6'h19 等待电源稳定
          ...
          // else 
          if (pwrup_state_trans_en && pgd_so_far) begin //pwrup_state_trans_en：上电流程切换使能（此处表示等待稳定超时，即电源已稳定）
            state_ns = SM_STEADY_PWROK;
          end
    end
