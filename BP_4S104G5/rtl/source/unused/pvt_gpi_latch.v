...
module pvt_gpi_latch   #(parameter TOTAL_BIT_COUNT  = 64,
                             DEFAULT_STATE          = 64'h0,
   ...
   output reg    [SEGMENT_MAX:SEGMENT_MIN] par_data_latch,// Parallel data output, latched by ld#
...
always@(posedge clk or negedge reset_n) begin
  if (!reset_n)
    par_data_latch <= DEFAULT_STATE;
  else if (par_load_out_n==1'b0)
    par_data_latch <= par_data;
end
