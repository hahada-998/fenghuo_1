...
module pwrseq_master #(
  ...
  input            clk,                     // clock
  input            reset,                   // reset
  input            cmu_fault_clear_rst,     //c00268 20220121
  ...
  input            p0_pwrbtn_n,              // SB power button input (same signal driven to SB PWRBTN)
  input            [1:0]pch_thermtrip_n,          // SB bound thermtrip signal (same signal driven to SB THERMTRIP)
  output reg       force_pwrbtn_n,           // forces SB to switch to S5 after power shutdown due to fault
  input            cpu_thermtrip_fault_det,  //20220110 c00268;idms:202201040006
  input            [5:0]power_seq_sm_fb,     //c00268 for hitless
  input            mux_sel,                  //20220915 d00412

  // Misc.
  input            xr_ps_en                  ,// system allowed to power on (Xreg's ps_enable)
  input            pwron_override_n          ,// power-on override
  input            interlock_broken          ,// interlock broken indicator
  input            allow_recovery            ,// allow power button press to recover from HALT_POWER_CYCLE
  input            aux_video_holdoff         ,// allow AUX video to hold turning on of system
  input            pgood_rst_mask            ,// from ADR module to mask shutdown events
  //input            bmc_ready_out_n           ,// 20220107 c00268;idms:202201040006    //20220528 d00412
  input            bmc_clr_stby_tmout_n      ,// 20220110 c00268;idms:202201040006
...
//  input            hold_pch_rsmrst,         // set to 1'b1 to stall power sequencer in state before RSMRST# is released
  ...
  output reg       cmu_fault_clear,          // c00268 20220121,for cmu 
  output reg   [5:0] power_seq_sm,             // copy of the state variable  //20220915 d00412
...
always @(posedge clk or posedge reset) begin
  if (reset)
    power_seq_sm_last <= power_seq_sm_fb;//SM_RESET_STATE; c00268 for hitless
  else if (t1us)
...
always @(posedge clk or posedge reset) begin
  if (reset)
    ...
    turn_system_on <= (xr_ps_en | ~pwron_override_n | turn_system_on)   &  // allowed to turn on
                      pch_slp4_n                                        &  // SB in S0 state
                      ~interlock_broken                                 &  // nothing is broken
                      ~aux_video_holdoff;
end
...
always @(posedge clk or posedge reset) begin
  if (reset)
    ...
    force_pwrbtn_n <=
      ~((           st_halt_power_cycle   &  fault_power                          // \ 1.
                                          & ~pf_on_wait_complete)              |  // /
        (           st_off_standby        &  fault_power                          // \ 4.
                                          &  (po_on_wait_complete)                // |
                                          &  assert_power_button                  // |
                                          & ~pch_slp4_n));                        // /
  end
...
*/
//20220109 c00268;idms:202201040006
edge_delay #(
  .CNTR_NBITS    (2)
...
//20220109 c00268;idms:202201040006,addr t_thermtrip_pwr_down
//assign rt_normal_pwr_down = ~pgood_rst_mask &( ~pch_slp4_n | ~turn_system_on); //17260 20200523
...
always @(posedge clk or posedge reset) begin
  if (reset)
    state <= SM_RESET_STATE;//power_seq_sm_fb;//SM_RESET_STATE; c00268 for hitless  //20220915 d00412
  else if (t1us)
    state <= state_ns;
end
...
//20220915 d00412
always @(posedge clk or posedge reset) begin //20220810 c10 ok
  if (reset)
...
always @(*) begin
  ...
  case (state)
    SM_RESET_STATE : begin       //00
	  if (power_seq_sm_fb==SM_OFF_STANDBY && (mux_sel == 1'b0))//08   //20220915 d00412
	    state_ns = SM_OFF_STANDBY;
	  else if(power_seq_sm_fb==SM_STEADY_PWROK && (mux_sel == 1'b0))//20  //20220915 d00412
	...
	SM_OFF_STANDBY : begin         //0X08
      if (any_pwr_fault_det & ~keep_alive_on_fault) begin  
	    ...
	    //20220106 c02268;idms:202201040006,add bmc_ready_out_n
    ...
    SM_STEADY_PWROK : begin         //6'h20
	  ...
	  else if (rt_thermtrip_pwr_down) begin    //20220109 c00268;idms:202201040006     
        state_ns = SM_CRITICAL_FAIL;           //20220109 c00268;idms:202201040006
//		rt_failure_detected_set = 1'b1;
      end
   ...
   SM_DISABLE_GRP_ATX : begin       //27
    ...
    SM_DISABLE_PS_ON : begin        //30
      ...
      if (pdn_watchdog_timeout)
        state_ns = ((any_pwr_fault_det & ~keep_alive_on_fault ) || (cpu_thermtrip_fault_det )) ? SM_DISABLE_S5_DEVICES : SM_OFF_STANDBY; //w17260 1209   
    end
      ////20220110 c00268;changed '(!pch_thermtrip_n)' to '(cpu_thermtrip_fault_det)' 
    SM_DISABLE_S5_DEVICES : begin    //31
      ...
      if (pdn_watchdog_timeout) begin
		...
		else if(cpu_thermtrip_fault_det) //20220110 c00268;changed '(!pch_thermtrip_n)' to '(cpu_thermtrip_fault_det)'  
    ...
    SM_AUX_FAIL_RECOVERY : begin      //33
	  ...
	  if(~cmu_fault_clear)begin       //c00268 20220121
      state_ns = SM_RSMRST_DISABLE;
      end
    end

    default : begin
      state_ns = SM_RESET_STATE;//power_seq_sm_fb;//SM_RESET_STATE; c00268 for hitless  //20220915 d00412
    end
  endcase
end
