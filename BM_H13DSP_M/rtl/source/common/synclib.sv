...
module SYNC_RESET_N_AASD(
    input   logic                   clk                 , 
    input   logic                   aaad_rst_n          , 
    output  logic                   aasd_rst_n    
); 
logic                               ff_s1               ;  // lint_checking FFCSTD off
...
module SYNC_RESET_SASD(
    input   logic                   clk                  , 
    input   logic                   aaad_rst             , 
    output  logic                   sasd_rst
);
logic                               ff_s1               ;  // lint_checking FFWNSR RSTDAT off
...
module SYNC_DATA_N #(parameter SIGCNT = 1) (
    input   logic                   clk                 ,
    input   logic                   rst_n               ,
    input   logic   [SIGCNT-1:0]    din                 ,
    output  logic   [SIGCNT-1:0]    dout
);
logic   [SIGCNT-1:0]                ff_s1               ;
...
module STABLE (
    input   logic                   clk                 ,
    input   logic                   aaad_sig            , 
    output  logic                   sasd_sig
);
logic   [SIGCNT-1:0]                ff_s1               ;  // lint_checking FFWASR FFWNSR MRSTDT off
logic   [SIGCNT-1:0]                ff_s2               ;  // lint_checking FFWASR FFWNSR MRSTDT off
...
logic                               ff_s1               ;  // lint_checking FFWNSR off
...
module PGM_DEBOUNCE_N #(
    ...
    input  logic                  timer_tick  ,  // 计数器使能信号
    input  logic [SIGCNT-1:0]     din         ,  // 输入信号
    output logic [SIGCNT-1:0]     dout           // 输出信号
);

// 内部寄存器和信号
logic [SIGCNT-1:0]                  nxt_s1, nxt, out_i    ; // 同步寄存器和输出寄存器
logic [SIGCNT-1:0][(NBITS-1):0]     cnt                   ; // 计数器
logic [SIGCNT-1:0]                  equal                 ; // 状态比较信号
...
module PGM_DEBOUNCE_GPO_N #(
    ...
    input  logic                  rst_n,       // 低电平有效复位
    input  logic [SIGCNT-1:0]     din,         // 输入信号
    input  logic [1:0]            select_delay,// 延迟选择信号
    input  logic [(NBITS-1):0]    ndelay0,     // 延迟值选项 0
    input  logic [(NBITS-1):0]    ndelay1,     // 延迟值选项 1
    input  logic [(NBITS-1):0]    ndelay2,     // 延迟值选项 2
    input  logic [(NBITS-1):0]    ndelay3,     // 延迟值选项 3
    input  logic                  timer_tick,  // 计数器使能信号
    input  logic                  disable_db,  // 禁用去抖动信号
    output logic [SIGCNT-1:0]     dout         // 输出信号
);
...
// 内部寄存器和信号
logic [(NBITS-1):0]                 ndelay              ; // 当前选定的延迟值
logic [SIGCNT-1:0][(NBITS-1):0]     cnt                 ; // 计数器
logic [SIGCNT-1:0]                  nxt_s1, nxt, out_i  ; // 同步寄存器和输出寄存器
logic [SIGCNT-1:0]                  equal               ; // 状态比较信号
